 ...
 * @param {boolean} [props.isModal=false] Aktiviert die kompaktere Overlay-Darstellung.
 * @returns {JSX.Element} Panel mit Formular.
 */
const NewEntryPage = ({ formState, onInputChange, onSubmit, isModal = false }) => {
  const tipsContent = (
    <>
  ...
  return (
    <>
      ...
      />
      <section className={`new-entry-layout${isModal ? " new-entry-layout--modal" : ""}`}>
        <section className={`panel panel--editor${isModal ? " panel--editor-compact" : ""}`}>
          <header className="panel-heading">
            <p className="panel-eyebrow">{isModal ? "Zentrierter Composer" : "Neue Notiz"}</p>
            <h2 className="panel-title">Neuer Eintrag</h2>
            <p className="panel-subtitle">
              {isModal
                ? "Halte Gedanken, Ideen oder Fortschritte fest, ohne die aktuelle Ansicht zu verlassen."
        ...
        {isModal ? (
          <details className="panel panel--aside panel--aside-compact panel-aside-disclosure">
            <summary className="panel-aside-disclosure__summary">
              <div className="panel-aside-disclosure__summary-copy">
                <p className="panel-eyebrow">Leitfaden</p>
                <strong>Schreibhilfe</strong>
              </div>
              <span className="panel-badge">3 Tipps</span>
            </summary>
            <div
              className="panel-aside-disclosure__body"
              aria-labelledby="new-entry-tips-title"
            >
              {tipsContent}
            </div>
          </details>
